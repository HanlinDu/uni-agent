...
for name in (
    ...
    "uni_agent.cache.lmcache_system_prefix_connector",
    "vllm",
    "transfer_queue",
):
...
kv_transfer_config=$(printf '{kv_connector:LMCacheSystemPrefixConnector,kv_connector_module_path:uni_agent.cache.lmcache_system_prefix_connector,kv_role:kv_both,kv_connector_extra_config:{lmcache.mp.host:%s,lmcache.mp.port:%s,lmcache.mp.mp_transfer_mode:%s}}' \
...
ray_job_cmd=(
    ...
    "+trainer.weight_update_hooks=[{path:uni_agent.cache.maybe_advance_extra_prefix_cache_epoch,required:true}]"
...
if grep -R -E "ExtraPrefixCache write allow" "${log_search_paths[@]}" >/dev/null 2>&1; then
    echo "Validation: found uni-agent extra prefix cache warmup write allow."
else
    echo "Validation failed: no uni-agent extra prefix cache write-allow policy log found." >&2
    validation_failed=1
fi

if grep -R -E "ExtraPrefixCache write deny .*reason=policy" "${log_search_paths[@]}" >/dev/null 2>&1; then
    echo "Validation: found uni-agent extra prefix cache rollout write deny."
else
    echo "Validation failed: no uni-agent extra prefix cache rollout write-deny policy log found." >&2
    validation_failed=1
fi

if grep -R -E "ExtraPrefixCache read allow" "${log_search_paths[@]}" >/dev/null 2>&1; then
    echo "Validation: found uni-agent extra prefix cache rollout read allow."
else
    echo "Validation failed: no uni-agent extra prefix cache read-allow policy log found." >&2
    validation_failed=1
fi
